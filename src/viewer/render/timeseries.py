...
def make_settings(stream: TimeSeries) -> TimeSeriesSettings:
    return TimeSeriesSettings()


def draw_settings(name: str, stream: TimeSeries, settings: TimeSeriesSettings):
    imgui.set_next_item_width(-1)
    _, settings.width = imgui.slider_float(
        f"##width_{name}", settings.width, 0.5, 3.0, "Line width: %.1f"
    )
    imgui.set_next_item_width(-1)
    _, settings.gain = imgui.slider_float(
        f"##gain_{name}", settings.gain, 0.1, 20.0, "Gain: %.1f"
    )
    imgui.set_next_item_width(-1)
    _, settings.spacing = imgui.slider_float(
        f"##spacing_{name}", settings.spacing, 0.0, 100.0, "Channel spacing: %.0f"
    )
...
def draw_plot(
    ...
    y_min, y_max = settings.y_limits(stream.n_channels)
    ...
    if implot.begin_plot(f"{stream.name}"):
        ...
        imspec = implot.Spec(line_weight=settings.width)
        ...
        for data, xstart, xscale in stream.iter_visible(
            ...
            for ch in range(stream.n_channels):
                ...
                ys = ys * settings.gain + ch * settings.spacing
